...
export default async function handler(
	req: NextApiRequest,
	res: NextApiResponse<Round | Omit<RoundToDownload,"Participant"|"Session">[]>
) {
	if (req.method === "POST") {
		const { body } = req;
	...
	if (req.method==="GET"){
		...
		if(relatedSessions.length===0){
			...
			allRounds = await prisma.round.findMany({include: {
				Participant: {
					include:{
						Session:true
					}
...
function flattenRound(round:(Round & {
    Participant: Participant & {
        Session: Session;
    };
})){
	const {Participant,Session, ...rest} = {...round.Participant, ...round.Participant.Session,...round};	
	return rest;
}
